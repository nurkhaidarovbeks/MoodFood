...
| # | Задача | Приоритет | Заметки |
|---|--------|-----------|---------|
| G.1 | **Push-уведомления** — подключить реальный push | Низкий | Экран Notifications есть, бэкенд эндпоинта нет. Сделать через `flutter_local_notifications` для water/meal reminders |
